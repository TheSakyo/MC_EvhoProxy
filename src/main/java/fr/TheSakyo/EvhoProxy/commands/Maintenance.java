...
public class Maintenance extends Command {
    ...
    String GI_COLOR = ChatColor.GRAY.toString() + ChatColor.ITALIC.toString();


	/**********************************************************/
	/* PARTIE COMMANDE POUR ACTIVER/DÉSACTIVER LA MAINTENANCE */
	/**********************************************************/
	...
	public void execute(CommandSender sender, String[] args) {
		...
		if(args.length == 0) {
				  ...
				  sender.sendMessage(new TextComponent(prefix_cmd + "whitelist" + ChatColor.WHITE + ": " + ChatColor.GRAY + "Affiche la Liste des Joueurs ayant accès à la Maintenance."));
			...
			if(args[0].equalsIgnoreCase("help")) { ProxyServer.getInstance().getPluginManager().dispatchCommand(sender, "maintenance"); }
				...
				if(main.maintenanceConfig.contains("maintenance_mode")) {
				    ...
				    if(main.maintenanceConfig.contains("maintenance_mode.proxy") && main.maintenanceConfig.getBoolean("maintenance_mode.proxy")) listServers.add("proxy");
					...
					} else sender.sendMessage(new TextComponent(prefixMaintenance + GI_COLOR + "Aucun Serveur(s) est en Maintenance !"));
				...
				if(listPlayers != null && !listPlayers.isEmpty()) {
					...
					sender.sendMessage(new TextComponent(ChatColor.AQUA.toString() + ChatColor.UNDERLINE.toString() + "Liste des Joueur(s) ayant accès à la Maintenance :"));
				...
				} else sender.sendMessage(new TextComponent(prefixMaintenance + GI_COLOR + "Aucun Joueur(s) ont accès à la Maintenance !"));
			...
			if(args[0].equalsIgnoreCase("add")) {
				...
				if(listPlayers != null && listPlayers.contains(args[1])) sender.sendMessage(new TextComponent(prefixMaintenance + ChatColor.YELLOW.toString() + ChatColor.BOLD.toString() + args[1] + ChatColor.RED + " a déjà accès à la Maintenance !"));
				else {
				...
				} else sender.sendMessage(new TextComponent(prefixMaintenance + ChatColor.YELLOW.toString() + ChatColor.BOLD.toString() + args[1] + ChatColor.RED + " n'a déjà pas accès à la Maintenance !"));
				...
				if(ProxyServer.getInstance().getServerInfo(args[1]) != null) {
					...
					if(main.maintenanceConfig.contains("maintenance_mode." + serverInfo.getName())) sender.sendMessage(new TextComponent(prefixMaintenance + ChatColor.YELLOW.toString() + ChatColor.BOLD.toString() + serverInfo.getName() + ChatColor.RED + " est dèjà en Maintenance !"));
					else {
					...
					if(args[1].equalsIgnoreCase("proxy")) {
						...
						if(main.maintenanceConfig.contains("maintenance_mode.proxy")) sender.sendMessage(new TextComponent(prefixMaintenance + prefixMaintenance + ChatColor.RED + "Le Serveur " + ChatColor.YELLOW.toString() + ChatColor.BOLD.toString() + "Proxy" + ChatColor.RED + " est dèjà en Maintenance !"));
						else {
				...
				if(ProxyServer.getInstance().getServerInfo(args[1]) != null) {
					...
					} else sender.sendMessage(new TextComponent(prefixMaintenance + ChatColor.YELLOW.toString() + ChatColor.BOLD.toString() + serverInfo.getName() + ChatColor.RED + " n'est pas en Maintenance !"));
					...
					if(args[1].equalsIgnoreCase("proxy")) {
						...
						} else sender.sendMessage(new TextComponent(prefixMaintenance + prefixMaintenance + ChatColor.RED + "Le Serveur " + ChatColor.YELLOW.toString() + ChatColor.BOLD.toString() + "Proxy" + ChatColor.RED + " n'est pas en Maintenance !"));
			...
			if(args[0].equalsIgnoreCase("motd")) {
				...
				main.configMOTD.set("MOTD_MAINTENANCE", main.methodCustom.format(combinedArgs) + ChatColor.RESET);
