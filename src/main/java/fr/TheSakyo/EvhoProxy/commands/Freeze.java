...
public class Freeze extends Command {
   ...
   /* Récupère la class "Main" et créer la commande */
		...
		if(args.length != 1) { sender.sendMessage(Type); }
			...
			if(ProxyServer.getInstance().getPlayer(args[0]) != null) {
				...
				if((sender instanceof ProxiedPlayer p)) {
					...
					freezedPlayer(target.getServer().getInfo(), p.getName(), target);
					return;
				}

				freezedPlayer(target.getServer().getInfo(), "console", target);
	...
	// Petite Méthode pour 'freeze' un joueur (envoit depuis les autres Serveurs) //
	private void freezedPlayer(ServerInfo server, String sender, ProxiedPlayer p) {
