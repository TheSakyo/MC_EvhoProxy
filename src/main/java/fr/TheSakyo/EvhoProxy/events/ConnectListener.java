...
public class ConnectListener implements Listener {
    ...
    public void OnPlayerLogin(PostLoginEvent e) {
		...
		ProxyServer.getInstance().getScheduler().schedule(main, new Runnable() {
            public void run() {
                ...
                // Récupère le titre a affiché en achieviement
                String title = ChatColor.GOLD.toString() + ChatColor.BOLD.toString() + ChatColor.ITALIC.toString() + " a rejoint le Serveur !";
                String description = "Un Administrateur vient de se connecté !"; // Récupère la petite description a affiché en achieviement

                // Créer un achieviement qui sera envoyé aux autres Serveurs (Message Personnalisé), si le joueur est bien dans un Serveur
                if(p.getServer() != null) {
    ...
    public void OnPlayerDisconnect(PlayerDisconnectEvent e) {
        ...
        if(p.getServer() != null) {
            ...
            String description = "Un Administrateur vient de se déconnecté !"; // Récupère la petite description a affiché en achieviement

            new PluginMessageListener(main).sendCustomDataCountPlayers(p, "all", true);

            // Créer un achieviement qui sera envoyé aux autres Serveurs (Message Personnalisé)
            Advancements.create(p.getServer().getInfo(), p, true, title, description, false, false, "TASK" , "ALWAYS", "ENTITY_WITHER_AMBIENT", true);
        }
                             /* ------------------------------------ */
    }
